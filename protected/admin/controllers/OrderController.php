...
class OrderController extends BaseController {
	...
	public function actionIndex(){
		$sql="select `{{order}}`.`oid`,`{{order}}`.`uid`,`{{order}}`.`mark`,`{{order}}`.`name`,`{{order}}`.`address`,`{{order}}`.`add_time`,`{{order}}`.`update_time`,`{{order}}`.`platform`,`{{order}}`.`pay_style`,`{{order}}`.`detail`,`{{order}}`.`amount`,`{{order}}`.`state`
		from `{{order}}`";
