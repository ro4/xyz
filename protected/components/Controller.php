...
class Controller extends CController
{
	 ...
	 * @param unknown $id
	 */
	public function getTopicByDataId($id){
		$sql="select `{{topic_data}}`.`topic_id`,`{{topic_data}}`.`data_id`,`{{topic}}`.`topic_title`,`{{topic}}`.`add_time`,`{{topic}}`.`discuss_count` from `{{topic}}` 
				left join `{{topic_data}}` on (`{{topic_data}}`.`topic_id`=`{{topic}}`.`id`) where `{{topic_data}}`.`data_id`=$id" ;
